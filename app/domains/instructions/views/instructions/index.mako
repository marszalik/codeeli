...
<div class="section-head">
  <h1>Instrukcje</h1>
  <p>Szablony promptów używane przez generator. Placeholdery: {{program_prompt}}, {{recipe}}, {{context}}, {{plan}}, {{task_description}}, {{filename}}.</p>
</div>
  ...
  % for item in instructions:
    <form method="post" action="/instructions/${item['id']}" class="panel">
      <div class="d-flex justify-content-between align-items-center gap-3">
        <label class="form-label flex-grow-1">Tytuł
      ...
      <button class="btn btn-outline-light btn-sm mt-3" type="submit">Zapisz instrukcję</button>
    </form>
  % endfor
</div>
